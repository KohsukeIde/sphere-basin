...
def _wait_for_upstream(
    ...
    *,
    ...
    while True:
        if upstream_status_path.exists():
            upstream = _load_status(upstream_status_path)
            status['upstream_state'] = upstream.get('state')
            _write_status(status_path, status)
            state = str(upstream.get('state'))
            if state == 'completed':
                return upstream
            if state in {'failed', 'cancelled'}:
                raise RuntimeError(f'upstream chain ended with state={state}')
...
def _build_canonical_cfg(
    *,
    ...
    cfg = {
        'experiment': {
            'name': 'a80_consistency_pathway_canonical',
            'dev_dir': 'workspace',
            'dist_mode': 'local',
            'gpu_groups': [[0], [1]],
        },
        ...
        'jobs': [
            ...
            {'job_dir': 'sphere-small-small-cifar-10-32px-a80-no-lat-con-causal', 'checkpoints': ['ep0024', 'ep0074']},
        ]
        + [
    ...
    out = workspace_root / 'research_summary' / 'a80_consistency_pathway_canonical.yaml'
...
def _build_official_eval_cfg(
    *,
    ...
    cfg = {
        'experiment': {
            'name': 'a80_consistency_pathway_official_eval',
            'dev_dir': 'workspace',
            'dist_mode': 'local',
            'gpu_groups': [[0, 1]],
        },
        ...
        'tasks': [
            ...
            {
                'job_dir': 'sphere-small-small-cifar-10-32px-a80-no-lat-con-causal',
                'ckpt_fname': 'ep0074.pth',
                'regime_name': 'shared-fixed',
                'cache_sampling_noise': True,
                'use_sampling_scheduler': False,
    ...
    out = workspace_root / 'research_summary' / 'a80_consistency_pathway_official_eval.yaml'
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding='utf-8')
    return out
...
def _write_compare_csv(
    *,
    ...
    variant_group_map = {
        ...
        'sphere-small-small-cifar-10-32px-a80-no-lat-con-causal': 'a80_no_lat_con',
    }
    for variant_name, job_dir in variant_job_map.items():
        variant_group_map[job_dir] = f'a80_{variant_name}'
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Wait for a80 no-lat-con completion, then run no-pix-con/recon-only follow-up.'
    )
    parser.add_argument(
        '--config',
        type=str,
        default=str(project_root() / 'configs' / 'a80_consistency_pathway.yaml'),
    )
    parser.add_argument(
        '--upstream-status',
        type=str,
        default=str(project_root() / 'workspace' / 'research_summary' / 'no_lat_con_status.json'),
    )
    ...
    upstream_status_path = Path(args.upstream_status).resolve()
    status_path = workspace_root / 'research_summary' / 'consistency_pathway_status.json'

    specs, meta = _build_loss_specs(cfg, workspace_root)
    new_jobs = [spec['target_job'] for spec in specs]
    variant_job_map = {
        str(spec['manifest']['variant']): str(spec['target_job'])
        for spec in specs
    }
    ...
    status: dict[str, Any] = {
        ...
        'steps': [],
        'state': 'waiting',
        'current_step': 'wait_for_a80_no_lat_con',
    }
    _write_status(status_path, status)

    try:
        status['steps'].append({'name': 'wait_for_a80_no_lat_con', 'state': 'running'})
        _write_status(status_path, status)
        upstream = _wait_for_upstream(
        ...
        status['current_step'] = 'train_consistency_pathway'
        status['steps'].append({'name': 'train_consistency_pathway', 'state': 'running'})
        _write_status(status_path, status)
        ...
        if not args.dry_run:
            ...
            pathway_canonical = workspace_root / 'research_summary' / 'consistency_pathway_canonical_master.csv'
            pathway_official = workspace_root / 'research_summary' / 'consistency_pathway_official_eval_master.csv'
            shutil.copy2(canonical_master, pathway_canonical)
            shutil.copy2(official_master, pathway_official)
            compare_path = workspace_root / 'research_summary' / 'consistency_pathway_compare.csv'
            _write_compare_csv(
            ...
            status['canonical_master_copy'] = str(pathway_canonical)
            status['official_eval_master_copy'] = str(pathway_official)
