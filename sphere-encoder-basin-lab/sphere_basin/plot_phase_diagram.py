...
def _filter_phase_rows(df: pd.DataFrame, forward_steps: int, tau_deg: float) -> pd.DataFrame:
    out = _to_numeric(df, ['alpha', 'fid', 'forward_steps', 'tau_deg', 'capture_mass', 'terminal_angle_mean_deg', 'train_dist_loss'])
...
def main() -> None:
    ...
    parser.add_argument('--tau-deg', type=float, default=2.0)
    ...
    phase = _filter_phase_rows(df, forward_steps=args.forward_steps, tau_deg=args.tau_deg)
    if phase.empty:
    ...
    axes[1].plot(phase['alpha'], phase['capture_mass'], marker='o')
    axes[1].set_xlabel('alpha (deg)')
    axes[1].set_ylabel(f'M_{args.forward_steps} (tau={args.tau_deg:g} deg)')
    axes[1].set_title(f'Capture vs alpha (T={args.forward_steps})')
    ...
    if 'capture_mass' in phase.columns and 'fid' in phase.columns:
        plt.figure(figsize=(6, 4))
        plt.scatter(phase['capture_mass'], phase['fid'])
        plt.xlabel(f'M_{args.forward_steps} (tau={args.tau_deg:g} deg)')
        plt.ylabel('gFID / FID')
        plt.title('FID vs basin capture')
        plt.tight_layout()
