...
from research.compat import build_analysis_loader
from research.train_dynamics import (
    compute_reconstruction_metrics,
    make_fixed_noise,
    run_lightweight_generation_eval,
    run_theory_probe,
    save_training_metrics_payload,
    sigma_max_from_angle,
)
...
parser.add_argument("--wandb_log_images", type=str2bool, default=True)
# --- dataset
parser.add_argument(
    "--dataset_name",
    type=str,
...
# --- research logging
parser.add_argument("--fixed_vis_batch_size", type=int, default=16)
parser.add_argument("--fixed_gen_seed", type=int, default=123)
parser.add_argument("--probe_interval", type=int, default=0, help="in epochs")
parser.add_argument("--probe_batch_size_per_rank", type=int, default=64)
parser.add_argument("--probe_num_prior_samples", type=int, default=0)
parser.add_argument("--probe_num_data_samples", type=int, default=0)
parser.add_argument("--probe_forward_steps", type=int, nargs="+", default=[1, 4])
parser.add_argument("--probe_taus_deg", type=float, nargs="+", default=[2.0])
parser.add_argument(
...
parser.add_argument("--probe_target_tau_deg", type=float, default=2.0)
parser.add_argument("--probe_target_noise_scaler", type=float, default=1.0)
parser.add_argument("--probe_cfg", type=float, default=1.0)
parser.add_argument(
    "--probe_cfg_position",
    type=str,
    default="combo",
    choices=["enc", "dec", "combo"],
)
parser.add_argument("--probe_use_sampling_scheduler", type=str2bool, default=False)
parser.add_argument("--probe_num_workers", type=int, default=4)
parser.add_argument("--probe_seed", type=int, default=1234)
parser.add_argument("--recon_eval_interval", type=int, default=0, help="in epochs")
parser.add_argument("--recon_num_samples", type=int, default=1024)
parser.add_argument("--light_eval_interval", type=int, default=0, help="in epochs")
parser.add_argument("--light_eval_num_samples", type=int, default=0)
parser.add_argument("--light_eval_batch_size_per_rank", type=int, default=64)
parser.add_argument("--light_eval_forward_steps", type=int, nargs="+", default=[1, 4])
parser.add_argument("--light_eval_cfg", type=float, default=1.0)
parser.add_argument(
    "--light_eval_cfg_position",
    type=str,
    default="combo",
    choices=["enc", "dec", "combo"],
)
parser.add_argument(
    "--light_eval_use_sampling_scheduler", type=str2bool, default=False
)
parser.add_argument("--light_eval_cache_sampling_noise", type=str2bool, default=False)
parser.add_argument("--light_eval_seed", type=int, default=4321)
parser.add_argument("--light_eval_num_snapshot_samples", type=int, default=64)
...
def resolve_wandb_mode(args) -> str:
    if not args.use_wandb:
        return "disabled"
    if args.wandb_mode != "auto":
        return args.wandb_mode
    has_key = bool(args.wandb_key or os.environ.get("WANDB_API_KEY"))
    return "online" if has_key else "offline"


def should_run_epoch_interval(epoch: int, interval: int) -> bool:
    if interval <= 0:
        return False
...
def main(args):
    ...
    args.research_dir = os.path.join(exp_dir, "research")
    for d in [args.vis_dir, args.ckpt_dir, args.research_dir]:
        if not ddp_rank0:
            continue
    ...
    log_research_path = os.path.join(args.research_dir, "training_metrics.jsonl")
    ...
    # logger
    args.sigma_max = sigma_max_from_angle(args.noise_sigma_max_angle)
    wandb_mode = resolve_wandb_mode(args)
    if args.use_wandb and ddp_rank0 and wandb_mode != "disabled":
        if wandb_mode == "online" and args.wandb_key:
            wandb.login(key=args.wandb_key, host="https://api.wandb.ai", relogin=True)
        wandb.init(
            name=args.wandb_name or job_dir,
            project=args.wandb_project or "sphere-encoder-basin-lab",
        ...
        wandb.config.update(
            {
                ...
                "latent_spatial_resolution": args.latent_resolution,
    ...
    wandb_active = args.use_wandb and wandb_mode != "disabled"
    ...
    fixed_vis_imgs, fixed_vis_clss = next(vis_loader)[:2]
    fixed_vis_count = min(args.fixed_vis_batch_size, fixed_vis_imgs.shape[0])
    fixed_vis_batch = (
        fixed_vis_imgs[:fixed_vis_count].clone(),
        fixed_vis_clss[:fixed_vis_count].clone(),
    )
    fixed_gen_noise = make_fixed_noise(
        model_without_ddp,
        batch_size=fixed_vis_count,
        device=device,
        seed=args.fixed_gen_seed,
    )
    fixed_gen_noise_alt = make_fixed_noise(
        model_without_ddp,
        batch_size=fixed_vis_count,
        device=device,
        seed=args.fixed_gen_seed + 1,
    )

    analysis_loader = None
    if args.probe_interval > 0 and args.probe_num_data_samples > 0:
        analysis_loader = build_analysis_loader(
            args,
            ddp_rank=ddp_rank,
            ddp_world_size=ddp_world_size,
            split="train",
            max_samples=args.probe_num_data_samples,
            batch_size_per_rank=args.probe_batch_size_per_rank,
    ...
    fid_stats_file_path = osp.join(
        args.dev_dir,
        "fid_stats",
        f"fid_stats_extr_{args.dataset_name}_{args.image_size}px.npz",
    )
    if args.light_eval_num_samples > 0 and not osp.exists(fid_stats_file_path):
        logger.warning(
            "lightweight eval disabled because fid stats are missing at %s",
            fid_stats_file_path,
        )
        args.light_eval_num_samples = 0

    init_vis_path = visualize(
        vis_loader=None,
        vis_batch=fixed_vis_batch,
        input_noise=fixed_gen_noise,
        alt_input_noise=fixed_gen_noise_alt,
    ...
    if ddp_rank0 and wandb_active and args.wandb_log_images and init_vis_path is not None:
        maybe_log_wandb(
            {
                "Media/Train_Grid": wandb.Image(init_vis_path),
                "epoch": cur_epoch if args.init_from == "resume" else 0,
                "step": global_step,
            },
            step=global_step,
    ...
    for epoch in range(cur_epoch, args.epochs):
        ...
        vis_path = None
        if should_run_epoch_interval(epoch, args.vis_interval):
            vis_path = visualize(
                vis_loader=None,
                vis_batch=fixed_vis_batch,
                input_noise=fixed_gen_noise,
            ...
            if ddp_rank0 and wandb_active and args.wandb_log_images and vis_path is not None:
                maybe_log_wandb(
                    {
                        "Media/Train_Grid": wandb.Image(vis_path),
                        "epoch": epoch,
                        "step": global_step,
                    },
                    step=global_step,
        ...
        run_probe = (
            analysis_loader is not None
            and args.probe_num_prior_samples > 0
            and should_run_epoch_interval(epoch, args.probe_interval)
        )
        run_recon = should_run_epoch_interval(epoch, args.recon_eval_interval)
        run_light_eval = (
            args.light_eval_num_samples > 0
            and should_run_epoch_interval(epoch, args.light_eval_interval)
        )

        if run_probe or run_recon or run_light_eval:
            model_without_ddp.eval()
            dynamics_metrics = {"epoch": epoch, "step": global_step}
            dynamics_payload = {
                "epoch": epoch,
                "step": global_step,
            ...
            if run_probe:
                theory_metrics, theory_payload = run_theory_probe(
                    model_without_ddp,
                    loader=analysis_loader,
                    device=device,
                    batch_size_per_rank=args.probe_batch_size_per_rank,
                    num_prior_samples=args.probe_num_prior_samples,
                    num_data_samples=args.probe_num_data_samples,
                    forward_steps=args.probe_forward_steps,
                    taus_deg=args.probe_taus_deg,
                    contraction_noise_scalers=args.probe_contraction_noise_scalers,
                    target_tau_deg=args.probe_target_tau_deg,
                    target_noise_scaler=args.probe_target_noise_scaler,
                    cfg=args.probe_cfg,
                    cfg_position=args.probe_cfg_position,
                    use_sampling_scheduler=args.probe_use_sampling_scheduler,
                    seed=args.probe_seed,
                )
                dynamics_metrics.update(theory_metrics)
                dynamics_payload["theory"] = theory_payload
            ...
            if run_recon:
                recon_metrics = compute_reconstruction_metrics(
                    ...
                    loader=test_loader,
                    device=device,
                    num_samples=args.recon_num_samples,
                    ptdtype=ptdtype,
                )
                dynamics_metrics.update(recon_metrics)
                dynamics_payload["reconstruction"] = recon_metrics

            if run_light_eval:
                generation_metrics, generation_payload = run_lightweight_generation_eval(
                    model_without_ddp,
                    exp_dir=exp_dir,
                    ...
                    device=device,
                    ptdtype=ptdtype,
                    num_eval_samples=args.light_eval_num_samples,
                    batch_size_per_rank=args.light_eval_batch_size_per_rank,
                    forward_steps=args.light_eval_forward_steps,
                    cfg=args.light_eval_cfg,
                    cfg_position=args.light_eval_cfg_position,
                    use_sampling_scheduler=args.light_eval_use_sampling_scheduler,
                    cache_sampling_noise=args.light_eval_cache_sampling_noise,
                    fid_stats_file_path=fid_stats_file_path,
                    epoch=epoch,
                    step=global_step,
                    seed=args.light_eval_seed,
                    num_snapshot_samples=args.light_eval_num_snapshot_samples,
                )
                dynamics_metrics.update(generation_metrics)
                dynamics_payload["generation"] = generation_payload
                if ddp_rank0 and wandb_active and args.wandb_log_images:
                    for fwd, snapshot_path in generation_payload.get(
                        "snapshots", {}
                    ).items():
                        if snapshot_path:
                            maybe_log_wandb(
                                {
                                    f"Media/LightEval_Gen_{fwd}step": wandb.Image(
                                        snapshot_path
                                    ),
                                    "epoch": epoch,
                                    "step": global_step,
            ...
            if ddp_rank0:
                append_log(file_path=log_research_path, entry=dynamics_metrics)
                payload_path = save_training_metrics_payload(
                    exp_dir=exp_dir,
                    epoch=epoch,
                    step=global_step,
                    payload=dynamics_payload,
                )
                logger.info("saved training dynamics payload: %s", payload_path)
                if wandb_active:
                    maybe_log_wandb(
                        dynamics_metrics,
                        step=global_step,
