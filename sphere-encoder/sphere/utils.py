...
def visualize(
    ...
    if vis_batch is None:
        assert vis_loader is not None
        imgs, clss = next(vis_loader)[:2]
    else:
        imgs, clss = vis_batch[:2]
    ...
    if gen_classes is not None:
        gen_clss = gen_classes[:N].to(device, non_blocking=True)
    elif class_of_interest is not None:
    ...
    with fsdp_ctx, ctx:
        ...
        # generation
        gen_y = gen_clss if model_without_ddp.use_modulation else None
        if input_noise is None and alt_input_noise is None:
            ...
            noise_a = input_noise if input_noise is not None else alt_input_noise
            noise_b = alt_input_noise if alt_input_noise is not None else noise_a
            noise_a = noise_a[:N].to(device)
            noise_b = noise_b[:N].to(device)
            gen_rand_1_step, gen_rand_n_step = model_without_ddp.edit(
                batch_size=N,
                y=gen_y,
                cfg=cfg,
                cfg_position=cfg_position,
            ...
            gen_clss_1_step, gen_clss_n_step = model_without_ddp.edit(
                batch_size=N,
                y=gen_y,
                cfg=cfg,
                cfg_position=cfg_position,
