    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        with autocast_ctx:
            ...
            if self.convnext is not None:
                inp_224 = F.interpolate(inp, size=224, mode="bilinear", antialias=True)
                tgt_224 = F.interpolate(tgt, size=224, mode="bilinear", antialias=True)
                inp_logits = self.convnext(
                    (inp_224 - self.imagenet_mean) / self.imagenet_std
                )
                tgt_logits = self.convnext(
                    (tgt_224 - self.imagenet_mean) / self.imagenet_std
                )
